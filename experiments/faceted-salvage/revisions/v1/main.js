...
scene.background = new THREE.Color('#c4b69e');
scene.fog = new THREE.Fog('#c4b69e', 46, 100);
...
const ground=new THREE.Mesh(new THREE.PlaneGeometry(250,250),new THREE.MeshStandardMaterial({color:'#b9aa8e',roughness:1}));
...
function reset(){world=makeWorld(collision);clearInput();demo=false;cfg.motion='auto';actorRoot.rotation.y=Math.PI;$('demo').textContent='演示行走';if(cfg.behavior==='follow')target.set(world.player.x,0,world.player.z);updateObjects();syncUI();}
$('demo').onclick=()=>{if(demo){demo=false;$('demo').textContent='演示行走';return;}reset();cfg.motion='auto';preset('follow');demo=true;$('demo').textContent='停止演示';};
...
$('screenshot').onclick=()=>{renderer.render(scene,camera);renderer.domElement.toBlob(b=>{if(b)download(b,'faceted-salvage.png');});};
...
function optimizeStatic(root) {
  ...
  root.traverse(o=>{if(!o.isMesh)return;const g=o.geometry.clone();g.applyMatrix4(o.matrixWorld);
    ...
    g.deleteAttribute('uv');g.deleteAttribute('uv1');g.deleteAttribute('color');const key=o.material.uuid;let b=buckets.get(key);if(!b){b={material:o.material,geometries:[]};buckets.set(key,b);}b.geometries.push(g);
  });
...
function updateObjects(){actorRoot.position.set(world.player.x,0,world.player.z);marker.position.set(world.player.x,.035,world.player.z);}
...
async function load(){
  try {
    ...
    const [env,character]=await Promise.all([
    ...
    for(const name of ['Idle','Walk'])if(!actions[name])throw Error('Missing animation: '+name+'; found '+Object.keys(actions));
...
function tick(now){
  ...
  if(ready){
    if(cfg.motion==='auto') {
      if(demo && (keys.size || [...taps.values()].some(t=>t>now) || world.player.z < -3)){demo=false;$('demo').textContent='演示行走';}
      ...
      if(v.lengthSq()>0){v.normalize();const moved=stepWorld(world,v.x*cfg.speed*dt,v.z*cfg.speed*dt);next=moved.distance>.00001?'Walk':'Idle';
  ...
  frames++;fpsTime+=realDt;if(fpsTime>.35){$('fps').textContent=Math.round(frames/fpsTime)+' FPS';frames=0;fpsTime=0;$('actor-state').textContent=({Idle:'待机',Walk:'行走'})[state]+(cfg.motion!=='auto'?' · 预览':'');$('position').textContent=`${world.player.x.toFixed(2)}, ${world.player.z.toFixed(2)}`;$('demo-status').textContent=demo?'演示中，方向键可接管':'自由行走 · 场景机关仅作陈设';}
}
// Read-only diagnostics for smoke verification, no connection to production game state.
window.facetedLab={snapshot:()=>({ready,actorState:state,player:{...world.player},camera:{...cfg,target:target.toArray()},animations:Object.keys(actions),drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,elapsed})};
