...
from salvage_details import wall_chunk
...
def build(out):
    ...
    for x in range(8):
        for y in range(8):
            ...
            slab=box('Individual worn slab',(xx,yy,-.055),(1.575,1.575,.15),floor,.023,.20)
            if random.random()<.24:
                for k in range(3):
                    px,py=xx+random.uniform(-.66,.66),yy+random.choice([-.755,.755])
                    mesh('Slab edge spall',[(px,py,.022),(px+.13,py,.022),(px+.09,py-math.copysign(.055,py-yy),.022)],[(0,1,2)],edge)
            if random.random()<.38:
    ...
    def wall(x,y,length,h,axis):
        ...
        for i in range(segments):
            ...
            o,top=wall_chunk('Fractured monolithic wall',loc[0],loc[1],width,.60,height,axis,stone,edge)
            if height>1 and i==0:
                for k in [-.23,.18]:
                    a=(loc[0]+k,loc[1],top-.16) if axis=='x' else (loc[0],loc[1]+k,top-.16)
                    b=(a[0]+.04,a[1]-.02,top+.14)
                    rod('Rebar exposed in fracture',a,b,.012,rust,6)
                    rod('Bent rebar end',b,(b[0]+.08,b[1]-.04,b[2]+.09),.012,rust,6)
            # Fracture runs vertically through the monolith, unlike masonry courses.
            z=height*.8;t=-width*.18
            for k in range(4):
                nt=t+random.uniform(-.10,.11);nz=z-random.uniform(.09,.21)
                if axis=='x':a,b=(loc[0]+t,y-.302,z),(loc[0]+nt,y-.303,nz)
                else:a,b=(x+.302,loc[1]+t,z),(x+.303,loc[1]+nt,nz)
                rod('Branching concrete fracture',a,b,.006,steel,4);z,t=nz,nt
    ...
    for x in [-6.1,-3.2,0,3.2,6.1]:
        box('Column foot plate',(x,6.85,.05),(.43,.46,.09),rust,.018,.3)
        for dx in [-.15,.15]:
            for dy in [-.16,.16]:rod('Column foundation anchor',(x+dx,6.85+dy,.10),(x+dx,6.85+dy,.145),.025,steel,6)
        ...
        if x<6:
            rod('Diagonal retaining brace',(x,6.87,1.25),(min(6.1,x+2.4),6.87,2.76),.045,rust,4)
        mesh('Upright triangular gusset',[(x-.11,6.70,.13),(x-.11,6.70,.64),(x-.11,6.41,.13)],[(0,1,2)],rust,.15)
        for z in [.5,1.8]:
    ...
    def cube_body():
        ...
        for s in [-1,1]:
            box('Recessed cargo side panel',(0,s*.676,.62),(1.09,.012,.87),steel,.018,.20)
            for x in [-.49,.49]:
                for z in [.25,.99]:rod('Cargo captive screw',(x,s*.685,z),(x,s*.699,z),.021,rust,6)
            box('Cargo handle pocket',(0,s*.689,.96),(.24,.015,.076),steel,.01)
            box('Cargo carry handle',(0,s*.71,.958),(.17,.025,.025),grey,.006,.1)
        for dx in [-.65,.65]:
        ...
        for dy,rot in [(-.705,(math.pi/2,0,0)),(.705,(math.pi/2,0,math.pi))]:stencil('3',(0,dy,.36),.53,ivory,rot)
    ...
    def l_body():
        for x,y in [(0,0),(1.6,0),(1.6,-1.6)]:
            ...
            for yy in [-.811,.811]:
                box('Recessed welded steel panel',(x,y+yy,.60),(1.35,.012,.85),rust,.014,.20)
                for dx in [-.64,.64]:
                    for z in [.22,1.0]:rod('L cargo rivet',(x+dx,y+yy,z),(x+dx,y+yy+math.copysign(.014,yy),z),.021,grey,6)
                for dx in [-.54,.54]:box('Vertical cargo panel stiffener',(x+dx,y+yy,.59),(.039,.028,.82),grey,.006,.25)
    ...
    def dummy():
        ...
        for x in [-.46,.46]:
            for y in [-.46,.46]:
                box('Dummy structural post',(x,y,.57),(.22,.22,.95),grey,.023,.35)
                for z in [.17,.96]:rod('Frame assembly bolt',(x,y+math.copysign(.112,y),z),(x,y+math.copysign(.125,y),z),.025,steel,6)
