...
def box(name,loc,size,material,bevel=.015,wear=0):
    ...
    if bevel:
        bmesh.ops.bevel(bm,geom=list(bm.edges),offset=min(bevel,min(size)*.49),segments=1,affect='EDGES',clamp_overlap=True)
    bm.normal_update();d=bpy.data.meshes.new(name);bm.to_mesh(d);bm.free()
    o=bpy.data.objects.new(name,d);bpy.context.scene.collection.objects.link(o);o.location=loc;o.data.materials.append(material);o['wear']=wear
    return o

def rod(name,a,b,r,material,vertices=10,r2=None,wear=0):
    a,b=Vector(a),Vector(b)
    length=(b-a).length;r2=r if r2 is None else r2
    vs=[(math.cos(i*math.tau/vertices)*rad,math.sin(i*math.tau/vertices)*rad,z) for rad,z in [(r,-length/2),(r2,length/2)] for i in range(vertices)]
    ...
    o=mesh(name,vs,fs,material,wear);o.location=(a+b)/2;o.rotation_euler=(b-a).to_track_quat('Z','Y').to_euler()
    return o
