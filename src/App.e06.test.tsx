...
describe('E06 developer playtest entry', () => {
  ...
  it('offers the accepted contrast before the harder experiment in the isolated laboratory', () => {
    render(<App />);
    ...
    fireEvent.change(screen.getByRole('combobox', { name: '关卡' }), { target: { value: e06Contrast.id } });
    ...
    const replay = searchSpatialExperiment(createGame(e06Contrast.board), { maximumStates: 200_000 });
    ...
    expect(loadCourseProgress(window.localStorage, masteryV2Catalog, 'lab')).toEqual([e06Contrast.id]);
    ...
    expect((screen.getByRole('combobox', { name: '关卡' }) as HTMLSelectElement).value).toBe(e06Prototype.id);
  }, 15_000);
